# QUY ƯỚC VĂN PHONG TRÌNH BÀY VĂN BẢN CTM
*(Tách từ CLAUDE.md — Cập nhật 17/05/2026 — Căn cứ file MauVanPhong.docx đã thống nhất)*

────────────────────────────────────────────────────────────

## 1. Bố cục quốc hiệu & tiêu ngữ

Trình bày theo bảng 2 cột trên đầu mỗi văn bản:

| Cột trái (căn trái) | Cột phải (căn giữa) |
|:---|:---:|
| **CÔNG TY CP ĐT XD & TM CÁT TƯỜNG MINH** | **CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM** |
| TRUNG TÂM GDNN & SHLX BÌNH THUẬN | **Độc lập – Tự do – Hạnh phúc** |
| | ────────────────── |

## 2. Số hiệu và địa danh ngày ký

- **Số hiệu:** `…/[Loại văn bản]-[Cơ quan ban hành]`
  - Quyết định: `…/QĐ-TGĐ-CTM`
  - Hợp đồng: `…/HĐ-GK-CTM`
- **Địa danh ngày ký:** Căn phải, in nghiêng — *Lâm Đồng, ngày … tháng … năm 20xx*

## 3. Tên văn bản

In hoa, **in đậm**, căn giữa. Dòng phụ (nếu có) in thường đậm, căn giữa.

```
QUYẾT ĐỊNH
Về việc ban hành Quy chế Hạch toán Kế toán Nội bộ
```

## 4. Phần căn cứ

Mỗi căn cứ là một gạch đầu dòng, kết thúc bằng dấu chấm phẩy. Dòng cuối (đề nghị/xét đề nghị) kết thúc bằng dấu phẩy, không có gạch đầu dòng.

```
- Căn cứ Luật Doanh nghiệp số 59/2020/QH14 ngày 17 tháng 6 năm 2020;
- Căn cứ Thông tư số 99/2025/TT-BTC ngày 27 tháng 10 năm 2025 của Bộ Tài chính;
Xét đề nghị của Phòng Tài chính – Kế toán,
```

## 5. Cấu trúc điều khoản

- **Điều:** `Điều X. Tiêu đề điều` — **in đậm**
- **Khoản:** `X.1.`, `X.2.` — không in đậm, thụt lề
- **Điểm:** `a)`, `b)`, `c)` — không in đậm, thụt lề thêm

## 6. Bảng biểu

- Có đường viền đầy đủ 4 phía trên tất cả các ô.
- Hàng tiêu đề: **in đậm**, nền xám nhạt (nếu có màu).
- Ghi chú bên dưới bảng bằng chữ nghiêng cỡ nhỏ.

## 7. Đường kẻ phân cách

```
────────────────────────────────────────────────────────────
```

## 8. Mã số văn bản kèm phụ lục

```
(Ban hành kèm theo Quyết định số: …/QĐ-TGĐ-CTM ngày … tháng … năm 20xx)
```
In nghiêng, căn giữa, đặt ngay dưới tên phụ lục.

## 9. Khối ký kết cuối văn bản — 2 cột

| *Nơi nhận:* | **TỔNG GIÁM ĐỐC** |
|:---|:---:|
| *- HĐQT (để biết);* | *(Ký, đóng dấu)* |
| *- Kế toán trưởng (để thực hiện);* | |
| *- Lưu VP, Phòng TC-KT.* | **VŨ HỒNG PHONG** |

## 10. Khối ký phụ lục — 3 cột

| NGƯỜI LẬP | KẾ TOÁN TRƯỞNG | P. GIÁM ĐỐC / TỔNG GIÁM ĐỐC |
|:---:|:---:|:---:|
| *(Ký, ghi rõ họ tên)* | *(Ký, ghi rõ họ tên)* | *(Ký, ghi rõ họ tên)* |
| | | **VŨ HỒNG PHONG** |

## 11. Font chữ và định dạng tổng thể

| Yếu tố | Quy định |
|:---|:---|
| Font chữ | Times New Roman |
| Cỡ chữ thông thường | 12–13pt |
| Căn lề | Căn đều 2 lề (justify) |
| Ghi chú / chú thích | In nghiêng |
| Tiêu đề chương / mục lớn | In hoa, in đậm |
| Người ký cao nhất | TỔNG GIÁM ĐỐC — VŨ HỒNG PHONG |

> **Bắt buộc:** Mọi văn bản chính thức CTM (quyết định, hợp đồng, quy chế, biên bản, phụ lục) đều tuân thủ quy ước này. Khi soạn file `.docx`: font Times New Roman, margin 2cm các phía, khổ A4.

────────────────────────────────────────────────────────────
*Tách từ CLAUDE.md Mục 8 — Phiên bản độc lập từ 17/05/2026*
*Quản lý: CFO-AI CTM*
